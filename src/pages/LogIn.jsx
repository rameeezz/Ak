...
import { NavLink, Link, useNavigate } from "react-router-dom";
import axios from "axios";
import { useGoogleLogin } from "@react-oauth/google"; // Import hook from Google login package
import "../css/LogIn.css";
...
function LogIn({ saveUser, userRole }) {
  ...
  // Trigger Google login manually with your custom button
  const googleLogin = useGoogleLogin({
    onSuccess: (tokenResponse) => handleGoogleLoginSuccess(tokenResponse.credential),
  ...
  function setUserInput(e) {
    let myUserInfo = { ...user };
    myUserInfo[e.target.name] = e.target.value.toLowerCase();
    setUser(myUserInfo);
  }
    ...
    <div className="background">
      <div className="login-form">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="checkbox-container mb-2" onClick={() => setChecked(!checked)}>
            <div className={`checkbox-box ${checked ? "checked" : ""}`}>
              {checked && <div className="checkmark">&#10003;</div>}
            </div>
            <label className="checkbox-label">Remember Me</label>
          ...
          <div className="d-flex mt-2">
            <span className="text-black textSizeForSpan">Don't have an account?</span>
            <NavLink to="/register" className="ms-2 sizeOfFontNav text-primary">
              Create Account
            </NavLink>
          </div>
          ...
          {/* Replace GoogleLogin with a custom button */}
          <div className="d-flex justify-content-center gap-2 mt-3">
            <button onClick={() => googleLogin()} className="custom-google-btn">
              Sign in with Google
            </button>
          </div>
        </form>
